...
1. **입력** 또는 **보내기**를 선택하여 메시지를 보냅니다. 마법사에서 에이전트 구성을 시작합니다.
1. 에이전트 이름을 제시하라는 메시지가 표시되면 `Product support` 입력 후 메시지를 보냅니다. 마법사가 에이전트의 이름을 업데이트합니다.
1. 마법사에서 에이전트에 대한 자세한 정보를 묻는 메시지를 표시합니다. 응답하는 대신 대화형 인터페이스 맨 위에서 **구성으로 건너뛰기**를 선택하여 에이전트의 개요 페이지를 보고 생성형 AI가 지금까지 구성한 내용을 검토합니다.
![구성으로 건너뛰기 단추를 강조 표시하는 에이전트 만들기에 대한 대화형 인터페이스 스크린샷](../Media/skip-to-configure-agent.png)

## 에이전트 구성 및 지침 정의

다음으로, 에이전트의 속성 및 메타데이터를 수동으로 업데이트하여 이 연습에 대한 일관된 결과를 확인합니다.

1. 생성형 AI 마법사가 에이전트에 정의한 **이름**을 검토합니다. 필요한 경우 이름을 업데이트하여 `Product support`이(가) 되도록 합니다.
1. **설명** 속성을 검토하고 `A product support agent that can answer queries about Contoso Electronics products`(으)로 업데이트합니다.
1. **지침** 텍스트 상자에서 기존 지침을 다음으로 바꿉니다.
...
1. 제안된 프롬프트는 생성형 AI를 사용하여 생성되었습니다. 예정된 연습에서 해당 프롬프트를 업데이트합니다.
...
1. 테스트 창 내의 텍스트 상자에 `What can you do?`을(를) 입력하고 메시지를 제출합니다.
1. 응답을 기다. 응답이 "Copilot 에이전트를 사용해 주셔서 감사합니다!"라는 텍스트로 시작하는 것을 확인합니다. 앞서 에이전트에 대해 정의한 지침에서 설명한 대로 적용됩니다.
    ...
    또한 에이전트에는 현재 지침이 있지만 아직 사용자 지정 지식 원본이나 작업이 없습니다. Contoso 제품에 대한 질문에 정확하게 답변할 수 있도록 에이전트를 아직 구성하지 않았습니다. 해당 작업은 다음 연습에서 수행합니다.

    > [!NOTE]
    > 에이전트를 편집해야 하는 경우 에이전트 개요 페이지의 **세부 정보** 섹션에서 **편집**을 선택합니다. 변경 내용을 저장합니다. 다시 테스트하기 전에 테스트 창 내에서 **새 테스트 세션 시작** 단추를 선택합니다.

## Microsoft 365 Copilot에 에이전트 게시

다음으로, Microsoft 365 Copilot에 에이전트를 게시합니다. **Product Support** 에이전트 개요 페이지에서 다음을 수행합니다.
...
1. **간단한 설명** 텍스트 상자에 자동으로 생성된 콘텐츠를 바꿔서 `Answers questions about Contoso Electronics products`을(를) 입력합니다.
1. 나머지 필드에 대한 기본 제안을 수락합니다.
1. **게시**를 선택합니다.
    ![게시 단추를 선택하기 전의 에이전트 게시 창 스크린샷](../Media/publish-window.png)
...
1. **공유 링크**에서 **복사**를 선택하여 에이전트의 공유 링크를 복사한 다음 **완료**를 선택합니다.
    ![복사 단추를 강조 표시하는 가용성 옵션 창 스크린샷](../Media/share-link-copy.png)
...
다음으로, Microsoft 365 Copilot에서 에이전트를 테스트하고 **몰입형** 및 **컨텍스트** 환경 모두에서 기능을 확인해 보겠습니다.

이전 단계를 따른 결과 현재 **몰입형** 에이전트 환경에 있습니다. 채팅 인터페이스 측면 창의 **에이전트** 섹션에서 현재 채팅 중인 에이전트로 **제품 지원**이 선택되어 있는지 확인합니다.
...
1. 사이드바의 **에이전트** 창 위에서 **새 채팅**을 선택하여 Microsoft 365 Copilot과 새 대화를 시작하고 **제품 지원** 에이전트와의 몰입형 채팅을 종료합니다.
...
1. 플라이아웃에서 **제품 지원**을 선택합니다. 메시지 상자 위의 상태 메시지를 확인합니다. **제품 지원과 채팅**이 표시됩니다. 이제 Copilot과의 대화 내에서 제품 지원 에이전트와 **컨텍스트 내**에서 채팅하고 있습니다. 즉, 에이전트가 Copilot과의 대화에서 컨텍스트를 고려할 수 있습니다.
...
1. 컨텍스트 환경을 종료하려면 상태 메시지에서 (X)를 선택합니다. 상태 메시지가 제거되고 더 이상 제품 지원 에이전트와 채팅하지 않음을 나타내는 메시지가 채팅 창에 표시됩니다. Copilot과 직접 대화를 계속할 수 있습니다.
